  ...
  pool.getConnection(function (err, con) {
    con.beginTransaction(async function (err) {
      ...
      try {
        ...
        if (!validated.error) {
          await functions.runTransactionQuery(`Update form set visit_type_id=${req.body.visit_type_id} where visit_id = ${req.body.visit_id} && deleted_at is null`, con)
          ...
          res.status(200).send({message: 'Instance Updated Successfully'})
        } else {
          con.rollback(function () {
            con.release()
            // Failure
